...
)
{
	try
	{
		...
		NTSTATUS retVal = Old_NtProtectVirtualMemory(ProcessHandle, BaseAddress, NumberOfBytesToProtect, NewAccessProtection, OldAccessProtection);
		if (retVal == STATUS_SUCCESS)
		{
			if ((NewAccessProtection & PAGE_EXECUTE_READ || NewAccessProtection & PAGE_EXECUTE_READWRITE) && Size != 37)
			{
				...
				{	
					...
					// Scan memory for internal whitelist
					if (!MemoryWhitelist((DWORD_PTR)Base, Size))
					{
	...
	)
{
	try
	{
		...
		NTSTATUS retVal = Old_NtWriteVirtualMemory(ProcessHandle, BaseAddress, Buffer, NumberOfBytesToWrite, NumberOfBytesWritten);

		if (retVal == STATUS_SUCCESS && NumberOfBytesToWrite != 4)
		{
			if (ROPTrace())
				LOG("s", "ROP_Detection", "CALLER_CHECK");

			if (ProcessHandle == GetCurrentProcess())
				strcpy_s(exePath, MAX_PATH, "<self>");
			else
				HandleToProcessPath(ProcessHandle, exePath);
			//log("NtAllocateVirtualMemory,%s,PAGE_EXECUTE_READWRITE", exePath);
			LOG("slS", "Target", exePath, "Size", NumberOfBytesToWrite, "Buffer", Buffer, 10);
		}
		return retVal;
	}
	catch (...)
	{
		LOG("s", "Exception", "NtWriteVirtualMemory");
		return -1;
	}
}
	...
	)
{
	try
	{
		...
		NTSTATUS retVal = Old_NtAllocateVirtualMemory(ProcessHandle, BaseAddress, ZeroBits, RegionSize, AllocationType, Protect);
		if (retVal == STATUS_SUCCESS)
		{
			if ((Protect & PAGE_EXECUTE_READ || Protect & PAGE_EXECUTE_READWRITE) && Size != 37 && Base != 0)
			{
				if (DirtyEsp())
					LOG("s", "ROP_Detection", "STATUS_STACK_BUFFER_OVERRUN");
				...
				if (AddrOutsideModule2((DWORD_PTR)Base) && !MemoryWhitelist((DWORD_PTR)Base, Size))
				{
					// determine what memory this will actually modify
					MEMORY_BASIC_INFORMATION lpBuffer;
					VirtualQuery(Base, &lpBuffer, sizeof(MEMORY_BASIC_INFORMATION));
					...
					{
						LOG("lllllS", "AccessProtection", Protect, "Size", Size, "BaseAddress", Base, "RegionSize", lpBuffer.RegionSize, "RegionBase", lpBuffer.BaseAddress, "Buffer", Base, 10);

						if (drop)
						{
							char outFile[MAX_PATH];
							sprintf_s(outFile, MAX_PATH, "C:\\drop\\%s_AVM_%x", "g_baseExe", lpBuffer.BaseAddress);
							HANDLE hFile = CreateFileA(outFile, GENERIC_WRITE, FILE_SHARE_READ, 0, CREATE_NEW, FILE_ATTRIBUTE_NORMAL, 0);
							if (hFile != INVALID_HANDLE_VALUE)
							{
								DWORD dWritten = 0;
	...
	{
		LOG("s", "Exception", "NtAllocateVirtualMemory");
		return -1;
	}
}
	...
	)
{
	try
	{
		...
		NTSTATUS retVal = Old_NtCreateSection(SectionHandle, DesiredAccess, ObjectAttributes, MaximumSize, SectionPageProtection, AllocationAttributes, FileHandle);
		if (retVal == STATUS_SUCCESS)
		{
			if (SectionPageProtection & PAGE_EXECUTE_READ || SectionPageProtection & PAGE_EXECUTE_READWRITE)
			{
				if (DirtyEsp())
					LOG("s", "ROP_Detection", "STATUS_STACK_BUFFER_OVERRUN");
	...
	{
		LOG("s", "Exception", "NtCreateSection");
		return -1;
	}

}

HOOKDEF(NTSTATUS, WINAPI, NtSuspendProcess,
	__in        HANDLE ProcessHandle
	)
{
	char exePath[MAX_PATH];

	NTSTATUS ret = Old_NtSuspendProcess(ProcessHandle);
...
{
	char exePath[MAX_PATH];
	BOOL retVal = Old_VirtualProtectEx(hProcess, lpAddress, dwSize, flNewProtect, lpflOldProtect);
	if (retVal && dwSize != 37)
	{
		if (flNewProtect == PAGE_EXECUTE_READWRITE)
		{
			if (hProcess == GetCurrentProcess())
				...
				HandleToProcessPath(hProcess, exePath);
			//log("NtAllocateVirtualMemory,%s,PAGE_EXECUTE_READWRITE", exePath);
			LOG("slS", "AccessProtection", "PAGE_EXECUTE_READWRITE", "Size", dwSize, "Buffer", lpAddress, 10);
		}
	}
	return retVal;
...
{
	BOOL retVal = Old_VirtualProtect(lpAddress, dwSize, flNewProtect, lpflOldProtect);
	if (retVal && flNewProtect == PAGE_EXECUTE_READWRITE && dwSize != 37)
	{
		LOG("slS", "AccessProtection", "PAGE_EXECUTE_READWRITE", "Size", dwSize, "Buffer", lpAddress, 10);
	}
	return retVal;
	...
	)
{
	...
	NTSTATUS retVal = Old_NtFreeVirtualMemory(ProcessHandle, BaseAddress, RegionSize, FreeType);
